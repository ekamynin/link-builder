...
def _load_collab_catalog(api_key: str) -> dict:
    """Load full Collaborator catalog. Cached 6h. Returns {norm_domain: site_dict}."""
    items, _ = fetch_all_sites(api_key, dr_min=0, traffic_min=0, da_min=0)
    catalog = {}
    for item in items:
        from collaborator_api import parse_site as _ps
        parsed = _ps(item)
        d = _norm(parsed["domain"])
        if d:
            catalog[d] = parsed
    return catalog
